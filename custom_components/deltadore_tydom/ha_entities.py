...
class HaWindowSensor(BinarySensorEntity, HAEntity):
	"""Representation of a passive window sensor (open/closed contact)."""
	...
	def __init__(self, device: TydomWindow, hass) -> None:
		...
		self._device = device
		self._device._ha_device = self
		self._attr_unique_id = f"{self._device.device_id}_binary_sensor"
		self._attr_name = self._device.device_name
		self._registered_sensors = []
	...
	@property
	def is_on(self) -> bool:
		"""Return True when the window is open (HA convention: on=open for WINDOW device class)."""
		if hasattr(self._device, "openState"):
	...
	@property
	def icon(self) -> str:
		"""Return the icon based on open/closed state."""
...
class HaDoorSensor(BinarySensorEntity, HAEntity):
	"""Representation of a passive door sensor (open/closed contact)."""
	...
	@property
	def is_on(self) -> bool:
		"""Return True when the door is open (HA convention: on=open for DOOR device class)."""
		if hasattr(self._device, "openState"):
			return getattr(self._device, "openState", None) != "LOCKED"
		elif hasattr(self._device, "intrusionDetect"):
			return bool(getattr(self._device, "intrusionDetect", False))
