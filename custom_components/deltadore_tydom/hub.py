...
class Hub:
	...
	async def _create_window_device(self, device: TydomWindow) -> None:
		...
		if any(
			...
			ha_device = HaWindowSensor(device, self._hass)
			self.ha_devices[device.device_id] = ha_device
	...
	async def _create_door_device(self, device: TydomDoor) -> None:
		...
		if any(
			...
			ha_device = HaDoorSensor(device, self._hass)
			self.ha_devices[device.device_id] = ha_device
			if self.add_binary_sensor_callback:
				self.add_binary_sensor_callback([ha_device])
	...
	async def update_ha_device(self, stored_device, device):
		...
		try:
			...
			# Only scan for new sensors if the device has gained new public attributes
			current_attr_count = sum(
				1 for k in stored_device.__dict__ if not k.startswith("_")
			)
			if current_attr_count > getattr(ha_device, "_last_device_attr_count", 0):
				ha_device._last_device_attr_count = current_attr_count
				new_sensors = ha_device.get_sensors()
				if new_sensors and self.add_sensor_callback is not None:
					LOGGER.debug(
