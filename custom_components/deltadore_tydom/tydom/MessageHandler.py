...
area_to_devices: dict = {}  # Map area_id -> [{"unique_id": str, "endpoint_id": int, "device_id": int}]
...
class MessageHandler:
	...
	async def parse_devices_metadata(self, parsed, transaction_id):
		...
		for device in parsed:
			...
			for endpoint in device["endpoints"]:
				...
				# Build area→device mapping from link field
				if "link" in endpoint and endpoint["link"].get("type") == "area":
					...
					if device_unique_id not in [
						...
						LOGGER.debug(
							"Area mapping: area_id=%s -> device %s",
							area_id,
							device_unique_id,
						)
		return []
	...
	async def parse_areas_data(self, parsed, transaction_id):
		"""Parse area data (PUT /areas/data from Tydom)."""
		LOGGER.debug("parse_areas_data : %s", parsed)
		devices = []
		for area_item in parsed:
			if not isinstance(area_item, dict):
				continue
			area_id = area_item.get("id")
			if area_id is None:
				continue
			if area_item.get("error", 0) != 0:
				continue
			data = {}
			for elem in area_item.get("data", []):
				if isinstance(elem, dict) and elem.get("validity") == "upToDate":
					data[elem["name"]] = elem["value"]
			if not data:
				continue
			for device_info in area_to_devices.get(area_id, []):
				...
				if device is not None:
					LOGGER.info(
						"Area data update (area_id=%s, device=%s, data=%s)",
						area_id,
						unique_id,
						data,
					)
