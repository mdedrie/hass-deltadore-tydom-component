...
class EndpointDiscovery:
    ...
    def build_digest_headers(self, nonce: str) -> str:
        ...
        try:
            ...
            chal["realm"] = (
                "ServiceMedia" if self.host == "mediation.tydom.com" else "protected area"
            )
            chal["qop"] = "auth"
    ...
    async def test_endpoint(self, endpoint: str, method: str = "GET") -> dict[str, Any]:
        ...
        if "{device_id}" in endpoint or "{endpoint_id}" in endpoint:
            ...
            if self.device_ids:
                ...
                endpoint = endpoint.replace("{device_id}", device_id).replace("{endpoint_id}", device_id)
            else:
                return {
                    "status": "skipped",
...
async def main():
    ...
    parser.add_argument(
        "--mac",
        required=True,
        help="Adresse MAC du gateway Tydom"
    )
    parser.add_argument(
        "--password",
